...
CITATION_PATTERNS = {
    "SCC": [r"\(\s*\d{4}\s*\)\s*\d+\s*SCC\s*\d+", r"\(\d{4}\)\d+SCC\d+"],
    "SCC_ONLINE": [r"\d{4}\s*SCC\s*OnLine\s*SC\s*\d+"],
    "AIR": [r"AIR\s*\d{4}\s*SC\s*\d+"],
    "SCR": [r"\[\s*\d{4}\s*\]\s*\d+\s*SCR\s*\d+"],
    "INSC": [r"\d{4}\s*INSC\s*\d+"],
    "SLP": [r"SLP\s*\(.*?\)\s*No\.?\s*\d+\s*of\s*\d{4}"],
    "ARTICLE": [r"Article\s+\d+[A-Z\-]*", r"Art\.?\s*\d+[A-Z\-]*"],
    "SECTION": [
        r"Section\s+\d+[A-Z\-\(\)]*\s*(?:of\s+the\s+[A-Za-z\s]+)?",
        r"Sec\.?\s*\d+[A-Z\-\(\)]*",
        r"u/s\.?\s*\d+[A-Z\-\(\)]*",
    ],
    "RULE": [r"Rule\s+\d+[A-Z\-]*", r"Order\s+\d+\s+Rule\s+\d+"],
}
...
def extract_citations(full_text=""):
    ...
    try:
                    ...
                    citations.append(
                        {
                            ...
                            "treatment": detect_treatment(full_text),
                            "canonical": True,
                        }
                    )
        ...
        return {"citations": citations, "count": len(citations), "confidence": 95}
        ...
        return {"citations": [], "count": 0, "confidence": 0}
