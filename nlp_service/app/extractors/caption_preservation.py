...
RAW_CAPTION_PATTERNS = [
    {
        "family": "CIVIL_APPEAL",
        "pattern": r"(Appeal\s*\((?:civil|criminal)\)\s*" r"[\d\-\/]+\s*of\s*\d{4})",
    },
    {
        "family": "CIVIL_APPEAL",
        "pattern": r"(Civil\s+Appeal\s+No\.?\s*" r"[\d\-\/]+\s*of\s*\d{4})",
    },
    {
        "family": "CRIMINAL_APPEAL",
        "pattern": r"(Criminal\s+Appeal\s+No\.?\s*" r"[\d\-\/]+\s*of\s*\d{4})",
    },
    {
        "family": "SPECIAL_LEAVE_PETITION_CIVIL",
        "pattern": r"(SLP\s*\(C\)\s*No\.?\s*" r"[\d\-\/]+\s*of\s*\d{4})",
    },
    {
        "family": "WRIT_PETITION_CIVIL",
        "pattern": r"(WP\s*\(C\)\s*No\.?\s*" r"[\d\-\/]+\s*of\s*\d{4})",
    },
    {
...
def preserve_raw_caption(header):
    ...
    header = normalize_ocr_legal_text(header)
    ...
    for entry in RAW_CAPTION_PATTERNS:
        ...
        try:
            ...
            caption = re.sub(r"\s+", " ", match.group(0)).strip().upper()

            family_data = PROCEEDING_FAMILY_ONTOLOGY.get(family, {})

            case_type = family_data.get("case_type")

            court_type = family_data.get("court_type")
