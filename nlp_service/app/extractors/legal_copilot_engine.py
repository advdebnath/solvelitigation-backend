...
def generate_legal_copilot_response(
    ...
    try:
        ...
        if isinstance(ai_argument_data, dict):
            ...
            result["arguments"] = ai_argument_data.get("arguments", [])
        ...
        if isinstance(contradiction_risk_data, dict):
            ...
            result["identified_risks"] = contradiction_risk_data.get(
                "identified_risks", []
            )
        ...
        if isinstance(legal_draft_data, dict):

            result["drafting_support"] = legal_draft_data
        ...
        summary.append(f"Dominant Issue: {result['dominant_issue']}")

        if result["strategy"]:

            summary.append(f"Strategy: {result['strategy']}")

        if result["predicted_outcome"]:

            summary.append(f"Likely Outcome: {result['predicted_outcome']}")

        if result["identified_risks"]:

            summary.append(f"Risks: {', '.join(result['identified_risks'])}")

        result["copilot_summary"] = " | ".join(summary)
        ...
        confidence += len(result["arguments"]) * 2

        confidence += len(result["recommended_precedents"]) * 2
        ...
        print("❌ Legal Copilot Error:", str(e))
