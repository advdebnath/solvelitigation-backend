...
def select_optimal_node(
    ...
    try:
        ...
        nodes = mesh_state.get(
            "nodes",
            {}
        )

        node_health = mesh_state.get(
            "node_health",
            {}
        )
        ...
        for node_name, node_info in nodes.items():
            ...
            health = node_health.get(
                node_name,
                0.50
            )

            load = COGNITION_LOAD_STATE[
                "node_loads"
            ].get(
                node_name,
                0.50
...
def detect_rebalance_requirement():
    ...
    try:
        ...
        for node, load in COGNITION_LOAD_STATE[
            "node_loads"
        ].items():
