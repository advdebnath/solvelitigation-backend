...
def build_litigation_lineage(canonical_entity, entity_hash, judgments):
    ...
    for doc in judgments:
        ...
        if category:
            lineage["legal_domains"].add(category)

        case_number = doc.get("caseNumber", "")

        if case_number:
            lineage["case_numbers"].append(case_number)
    ...
    if (
        ...
        lineage["litigation_profile"]["government_entity"] = True
    ...
    if lineage["cases_found"] >= 3:
        ...
        lineage["litigation_profile"]["repeat_litigant"] = True
    ...
    if lineage["cases_found"] >= 20:
        ...
        lineage["litigation_profile"]["risk_level"] = "HIGH"

    elif lineage["cases_found"] >= 5:

        lineage["litigation_profile"]["risk_level"] = "MODERATE"

    # -----------------------------------------------------
    # 🔥 SERIALIZATION
    # -----------------------------------------------------

    lineage["courts"] = list(lineage["courts"])

    lineage["roles"] = list(lineage["roles"])

    lineage["legal_domains"] = list(lineage["legal_domains"])

    lineage["judges"] = list(lineage["judges"])
