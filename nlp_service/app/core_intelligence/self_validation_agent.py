...
def run_self_validation(
    ...
    try:
        ...
        constitutional_weight_value = adaptive_reasoning.get(
            "constitutional_weight", 0.5
        )

        persuasive_force_value = adaptive_reasoning.get("persuasive_force_score", 0.5)

        outcome_confidence_value = semantic_outcome_prediction.get(
            "outcome_confidence", 0.5
        )

        litigation_risk_value = semantic_outcome_prediction.get(
            "litigation_risk_score", 0.5
        )
        ...
        prediction_alignment_score = round(
            max(0.0, 1 - abs(persuasive_force_value - outcome_confidence_value)), 2
        )

        doctrine_prediction_conflict = False

        if constitutional_weight_value >= 0.7 and litigation_risk_value >= 0.7:
        ...
        self_validation.update(
            {
                ...
                "prediction_alignment_score": prediction_alignment_score,
                "doctrine_prediction_conflict": doctrine_prediction_conflict,
